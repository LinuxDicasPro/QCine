...
VlcControlVideo::VlcControlVideo(VlcMediaPlayer *player, const QString &language, QObject *parent)
    : QObject(parent), _vlcMediaPlayer(player), _vlcVideo(player->video()) {
...
void VlcControlVideo::updateSubtitles() {
    ...
    _vlcVideo->setSubtitle(_descSub.value(action->text()));
}
...
void VlcControlVideo::updateVideo() {
    ...
    _vlcVideo->setTrack(_descVideo.value(action->text()));
}
