...
VlcVideo::VlcVideo(VlcMediaPlayer *player) : QObject(player), _vlcMediaPlayer(player->core()) {}

VlcVideo::~VlcVideo() = default;
...
void VlcVideo::setSubtitleFile(const QString &subtitle) {
    if (_vlcMediaPlayer && libvlc_media_player_has_vout(_vlcMediaPlayer)) {
        libvlc_media_player_add_slave(_vlcMediaPlayer, libvlc_media_slave_type_subtitle, subtitle.toUtf8().data(), true);
...
void VlcVideo::showMarquee(const QString &text, int x, int y, int timeout, int opacity, int size,
                           int color, int refresh) {
    if (_vlcMediaPlayer && libvlc_media_player_has_vout(_vlcMediaPlayer)) {
...
//        libvlc_toggle_teletext(_vlcMediaPlayer);
        libvlc_video_set_teletext(_vlcMediaPlayer, 100);
        VlcError::showErrmsg();
    }
}
