...
VlcAbstractVideoFrame::VlcAbstractVideoFrame(int planeCount) : width(0), height(0) {
    planes.resize(planeCount);
    planeSizes.resize(planeCount);
}

VlcAbstractVideoFrame::~VlcAbstractVideoFrame() = default;
...
bool VlcAbstractVideoFrame::isValid() const {
    return width > 0 && height > 0;
}

void VlcAbstractVideoFrame::setPitchesAndLines(const unsigned *pitches, const unsigned *lines) {
