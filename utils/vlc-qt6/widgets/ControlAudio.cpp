...
VlcControlAudio::VlcControlAudio(VlcMediaPlayer *player, const QString &language, QObject *parent)
    : QObject(parent), _vlcAudio(player->audio()), _vlcMediaPlayer(player) {
...
void VlcControlAudio::update() {
    ...
    _vlcAudio->setTrack(_desc.value(action->text()));
}
