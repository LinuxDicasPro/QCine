...
VlcVideoStream::VlcVideoStream(Vlc::RenderFormat format, QObject *parent)
    : QObject(parent), _format(format) {}

VlcVideoStream::~VlcVideoStream() {
    if (_player) unsetCallbacks(_player);
}
...
unsigned VlcVideoStream::formatCallback(char *chroma, unsigned *width, unsigned *height,
...
void VlcVideoStream::unlockCallback(void *picture, void *const *planes) {
    ...
    if (frameNo >= _frames.size()) return; // LCOV_EXCL_LINE
...
std::shared_ptr<VlcAbstractVideoFrame> VlcVideoStream::cloneFrame(const std::shared_ptr<VlcAbstractVideoFrame>& frame) {
    switch (_format) {
    case Vlc::YUVFormat:
