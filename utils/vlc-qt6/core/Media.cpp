...
VlcMedia::VlcMedia(const QString &location, bool localFile, VlcInstance *instance) : QObject(instance) {
    initMedia(location, localFile, instance);
}

VlcMedia::VlcMedia(const QString &location, VlcInstance *instance) : QObject(instance) {
...
void VlcMedia::initMedia(const QString &location, bool localFile, VlcInstance *instance) {
...
bool VlcMedia::parsed() const {
    int parsed = libvlc_media_get_parsed_status(_vlcMedia);
...
void VlcMedia::parse() {
    libvlc_media_parse_with_options(_vlcMedia, libvlc_media_parse_local, -1);
...
VlcStats *VlcMedia::getStats() {
    auto *coreStats = new libvlc_media_stats_t;
    auto *stats = new VlcStats;
...
Vlc::State VlcMedia::state() const {
    libvlc_state_t state = libvlc_media_get_state(_vlcMedia);
    VlcError::showErrmsg();
...
QString VlcMedia::duplicate(const QString &name, const QString &path, const Vlc::Mux &mux,
                            const Vlc::AudioCodec &audioCodec, const Vlc::VideoCodec &videoCodec) {
    return record(name, path, mux, audioCodec, videoCodec, true);
}

QString VlcMedia::duplicate(const QString &name, const QString &path, const Vlc::Mux &mux,
                            const Vlc::AudioCodec &audioCodec, const Vlc::VideoCodec &videoCodec,
                            int bitrate, int fps, int scale) {
...
QString VlcMedia::record(const QString &name, const QString &path, const Vlc::Mux &mux,
                         const Vlc::AudioCodec &audioCodec, const Vlc::VideoCodec &videoCodec, bool duplicate) {
    QString option1, option2, parameters;
    QString l = QDir::toNativeSeparators(path + "/" + name);
...
void VlcMedia::libvlc_callback(const libvlc_event_t *event, void *data) {
    auto *core = static_cast<VlcMedia *>(data);
    ...
    case libvlc_MediaDurationChanged:
        emit core->durationChanged((int)event->u.media_duration_changed.new_duration);
        break;
