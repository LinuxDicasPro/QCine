...
public:
     ...
     */
    explicit VlcVideoStream(Vlc::RenderFormat format, QObject *parent = nullptr);
    ~VlcVideoStream() override;

    /*!
        \brief Rendering format
        \return current rendering format
     */
    [[nodiscard]] Vlc::RenderFormat format() const { return _format; } // LCOV_EXCL_LINE
     ...
     */
    [[nodiscard]] std::shared_ptr<const VlcAbstractVideoFrame> renderFrame() const { return _renderFrame; } // LCOV_EXCL_LINE
...
private:
    ...
    std::shared_ptr<VlcAbstractVideoFrame> cloneFrame(const std::shared_ptr<VlcAbstractVideoFrame>& frame);
