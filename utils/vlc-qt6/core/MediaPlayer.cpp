...
void VlcMediaPlayer::setTime(int time) {
    if (!(state() == Vlc::Buffering || state() == Vlc::Playing || state() == Vlc::Paused))
        return;
...
Vlc::State VlcMediaPlayer::state() const {
    ...
    libvlc_state_t state = libvlc_media_player_get_state(_vlcMediaPlayer);
    VlcError::showErrmsg();
...
void VlcMediaPlayer::libvlc_callback(const libvlc_event_t *event, void *data) {
    auto *core = static_cast<VlcMediaPlayer *>(data);
    ...
    case libvlc_MediaPlayerTimeChanged:
        emit core->timeChanged((int)event->u.media_player_time_changed.new_time);
        break;
    ...
    case libvlc_MediaPlayerLengthChanged:
        emit core->lengthChanged((int)event->u.media_player_length_changed.new_length);
        break;
    ...
    if (tracksCount > 0) {
        for (unsigned i = 0; i < tracksCount; i++) {
            ...
            if (track->i_type == libvlc_track_video && track->i_id == 0) {
                qDebug() << "Orientação: " <<  track->video->i_orientation;
