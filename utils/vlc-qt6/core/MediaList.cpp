...
VlcMediaList::VlcMediaList(VlcInstance *instance) : QObject(instance) {
...
int VlcMediaList::indexOf(VlcMedia *media) {
    return (int)_list.indexOf(media);
}
...
void VlcMediaList::insertMedia(VlcMedia *media, int index) {
    lock();
...
void VlcMediaList::libvlc_callback(const libvlc_event_t *event, void *data) {
    auto *core = static_cast<VlcMediaList *>(data);
