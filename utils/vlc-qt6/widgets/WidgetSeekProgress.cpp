...
VlcWidgetSeekProgress::VlcWidgetSeekProgress(VlcMediaPlayer *player, QWidget *parent)
    : VlcWidgetSeek(player, new QProgressBar(), true, parent), _lock(false) {
    _progress->setMaximumHeight(15);
}

VlcWidgetSeekProgress::VlcWidgetSeekProgress(QWidget *parent)
: VlcWidgetSeek(new QProgressBar(), true, parent), _lock(false) {
    _progress->setMaximumHeight(15);
}

VlcWidgetSeekProgress::~VlcWidgetSeekProgress() = default;
...
void VlcWidgetSeekProgress::wheelEvent(QWheelEvent *event) {
    ...
    // tod0: delta ??? now set pixelDelta
    if (event->pixelDelta().x() > 0)
        _vlcMediaPlayer->setTime(int(_vlcMediaPlayer->time() + _vlcMediaPlayer->length() * 0.01));
    else
        _vlcMediaPlayer->setTime(int(_vlcMediaPlayer->time() - _vlcMediaPlayer->length() * 0.01));
}

void VlcWidgetSeekProgress::setSliderWidget(QWidget *slider, bool updateSlider) {
    Q_UNUSED(slider)
...
void VlcWidgetSeekProgress::updateEvent(const QPoint &pos) {
    ...
    float click = (float)pos.x() - (float)_progress->pos().x();
    float op = (float)_progress->maximum() / (float)_progress->width();
    float newValue = click * op;

    _vlcMediaPlayer->setTime((int)newValue);
    _progress->setValue((int)newValue);
}
