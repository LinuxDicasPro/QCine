...
VlcEqualizer::VlcEqualizer(VlcMediaPlayer *vlcMediaPlayer) : QObject(vlcMediaPlayer),
...
uint VlcEqualizer::presetCount() {
    return libvlc_audio_equalizer_get_preset_count();
}

QString VlcEqualizer::presetNameAt(uint index) {
...
void VlcEqualizer::setEnabled(bool enabled) {
    if (enabled && _vlcEqualizer != nullptr) {
        ...
        libvlc_media_player_set_equalizer(_vlcMediaPlayer->core(), nullptr);
    }
}
