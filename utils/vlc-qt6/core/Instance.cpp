...
void logCallback(void *data, int level, const libvlc_log_t *ctx, const char *fmt, va_list args) {
    ...
    auto *instance = static_cast<VlcInstance *>(data);
...
VlcInstance::VlcInstance(const QStringList &args, QObject *parent) : QObject(parent),
...
// Convert arguments to required format
    ...
    _vlcInstance = libvlc_new((int)args.count(), argv);
...
void VlcInstance::setAppId(const QString &id, const QString &version, const QString &icon) {
...
QList<VlcModuleDescription *> VlcInstance::audioFilterList() const {
    ...
    do {
        auto *module = new VlcModuleDescription(VlcModuleDescription::AudioFilter, list->psz_name);
...
QList<VlcModuleDescription *> VlcInstance::videoFilterList() const {
    ...
    do {
        auto *module = new VlcModuleDescription(VlcModuleDescription::VideoFilter, list->psz_name);
        module->setLongName(list->psz_longname);
        module->setShortName(list->psz_shortname);
        module->setHelp(list->psz_help);
        videoFilters << module;
