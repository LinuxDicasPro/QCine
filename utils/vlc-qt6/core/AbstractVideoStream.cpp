...
VlcAbstractVideoStream::VlcAbstractVideoStream() = default;
...
void VlcAbstractVideoStream::unsetCallbacks(VlcMediaPlayer *player) {
    if (player) {
        libvlc_video_set_callbacks(player->core(), nullptr, nullptr, nullptr, nullptr);
        libvlc_video_set_format_callbacks(player->core(), nullptr, nullptr);
    }
}

void *VlcAbstractVideoStream::lockCallbackInternal(void *opaque, void **planes) {
    return P_THIS->lockCallback(planes);
}

void VlcAbstractVideoStream::unlockCallbackInternal(void *opaque, void *picture, void *const *planes) {
    P_THIS->unlockCallback(picture, planes);
}

void VlcAbstractVideoStream::displayCallbackInternal(void *opaque, void *picture) {
    P_THIS->displayCallback(picture);
}

unsigned VlcAbstractVideoStream::formatCallbackInternal(void **opaque, char *chroma, unsigned *width, unsigned *height,
